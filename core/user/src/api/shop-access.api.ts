...
export const shopAccessApi = {
  getMyAccess: async (): Promise<ShopAccess> => {
    const response = await apiClient.get<ApiResponse<ShopAccess>>(SHOP_ACCESS_ENDPOINTS.ME_ACCESS);
  ...
  updatePolicy: async (shopId: string, body: UpdateShopRbacPolicyRequest): Promise<void> => {
