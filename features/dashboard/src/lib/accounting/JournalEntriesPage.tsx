...
export function JournalEntriesPage() {
  ...
  const [data, setData] = useState<JournalEntriesPageResponse | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const res = await accountingApi.journals({
  ...
  }, [notifyError, source, from, to, page]);

  const entries = useMemo<JournalEntryResponse[]>(
    () => data?.entries ?? [],
    [data]
  );
