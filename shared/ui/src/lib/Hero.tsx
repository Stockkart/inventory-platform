...
import { videosApi } from '@inventory-platform/api';
...
export function Hero() {
  ...
  const [demoVideo, setDemoVideo] = useState<TutorialVideoResponse | null>(null);
  ...
  const handleWatchDemo = async () => {
    ...
    setDemoLoading(true);
    try {
      const video = await videosApi.getByKey(DEMO_VIDEO_KEY);
      setDemoVideo(video);
      setDemoOpen(true);
    } catch {
      try {
        const fallback = await videosApi.getByKey('demo');
